...
void test_deviceCallable_Do_solver() {
    std::cout << "Starting Device callable DO solver " << std::endl;
    using timer = std::chrono::high_resolution_clock;
    using Device = Kokkos::DefaultExecutionSpace;
...
//This tests the device callable american solver
void test_deviceCallable_Do_solver_american() {
    std::cout << "Starting Device callable american solver " << std::endl;
    using timer = std::chrono::high_resolution_clock;
    using Device = Kokkos::DefaultExecutionSpace;
    ...
    auto t_end = timer::now();
    std::cout << "Parallel DO American time: "
              << std::chrono::duration<double>(t_end - t_start).count()
              << " seconds" << std::endl;
...
//this tests a device callable dividend solver
void test_deviceCallable_Do_solver_dividend() {
    std::cout << "Starting Device callable DO solver Dividend" << std::endl;
    using timer = std::chrono::high_resolution_clock;
    using Device = Kokkos::DefaultExecutionSpace;
    ...
    auto t_end = timer::now();
    std::cout << "Parallel DO Dividend time: "
              << std::chrono::duration<double>(t_end - t_start).count()
              << " seconds" << std::endl;
...
//this tests a device callable american solver with dividends
void test_deviceCallable_Do_solver_american_dividend() {
    std::cout << "Starting Device callable DO solver Dividend" << std::endl;
    using timer = std::chrono::high_resolution_clock;
    using Device = Kokkos::DefaultExecutionSpace;
    ...
    for(int inst = 0; inst < min(2,nInstances); ++inst) {
        // Create host mirrors for the grid views
        auto h_Vec_s = Kokkos::create_mirror_view(hostGrids[inst].device_Vec_s);
        auto h_Vec_v = Kokkos::create_mirror_view(hostGrids[inst].device_Vec_v);
        Kokkos::deep_copy(h_Vec_s, hostGrids[inst].device_Vec_s);
