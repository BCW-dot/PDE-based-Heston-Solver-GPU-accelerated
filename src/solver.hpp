...
void DO_scheme_dividend_shuffled_with_surface_tracking_put(
    ...
    for(int n = 1; n <= N; n++) {
        ...
        while(!div_dates.empty() && t <= div_dates[0] && div_dates[0] < (n+1) * delta_t) {
            ...
            // Create temporary storage for new values
            ViewType U_new("U_new", U.extent(0));
            Kokkos::deep_copy(U_new, U);  // Initialize with current values
                    ...
                    for(int i = 0; i <= m1; i++) {
                        ...
                        if(new_s > 0) {
                            ...
                            if(idx > 0 && idx < m1 + 1) {
                                ...
                                double val_low = U(offset + idx-1);
                                double val_high = U(offset + idx);
                                double new_value = (1.0 - weight) * val_low + weight * val_high;
                                
                                // For puts: add the difference
                                U_new(offset + i) += new_value - U(offset + i);
                            }
                            else if(idx == 0) {
                                // Left extrapolation
                                double new_value = U(offset);
                                U_new(offset + i) += new_value - U(offset + i);
                            }
                            else {
                                // Right extrapolation
                                double new_value = U(offset + m1);
                                U_new(offset + i) += new_value - U(offset + i);
                            }
                        }
                    }
...
template<class ViewType, class BoundaryType>
void DO_scheme_american_dividend_shuffle_with_lambda_tracking_put(
    const int m,                    
    const int m1,                    
    const int m2,                    
    const int N,                     
    ...
    for (int n = 1; n <= N; n++) {
        ...
        Kokkos::parallel_for("early_exercise_check", m, KOKKOS_LAMBDA(const int i) {
            ...
            if(i % (m1 + 1) == 0) {  // This hits exactly S_max entries at all variance levels
                lambda_bar(i) = 0.0;
            }
            if(i % (m1 + 1) == 1) {  // This hits exactly S_max entries at all variance levels
                lambda_bar(i) = 0.0;
            }
            if(i % (m1 + 1) == 2) {  // This hits exactly S_max entries at all variance levels
                lambda_bar(i) = 0.0;
            }
        });
