...
void test_calibration_european(){
    ...
    */
    ...
    std::cout << "Total time after Updating parameters: "
              << std::chrono::duration<double>(t_end_second - iter_start).count()
              << " seconds" << std::endl;
    ...
    double total_time = std::chrono::duration<double>(t_end_second - iter_start).count();
    out << "# " << num_strikes 
...
void test_calibration_dividends(){
    ...
    */
    ...
    std::cout << "Strikes: ";
    for(int i = 0; i < num_strikes; ++i) {
        strikes[i] = S_0 * 0.7 + i;//S_0 * (0.5 + i * 0.01); //S_0 - num_strikes + i;  // Strikes
        std::cout << strikes[i] << ", ";
    }
    std::cout << "" << std::endl;
...
void test_calibration_american_dividends(){
    ...
    std::cout << "Strikes: ";
    for(int i = 0; i < num_strikes; ++i) {
        strikes[i] = S_0 * 0.7 + i * 1.0;//S_0 * (0.5 + i * 0.01); //S_0 - num_strikes + i;  // Strikes
        std::cout << strikes[i] << ", ";
    }
    std::cout << "" << std::endl;
...
This calibrates multiple maturity american options with dividents
...
//This calibrates to american call options with dividends across multiple maturities
void test_calibration_american_divident_multi_maturity(){
    std::cout<< "Testing american dividnet multi maturity" << std::endl;
    using Device = Kokkos::DefaultExecutionSpace;
    using timer = std::chrono::high_resolution_clock;
    ...
    */
    ...
    //CAN NOT USE BSM FOR MARKET PRICES 
    ...
    for(int iter = 0; iter < max_iter && !converged; iter++) {
        ...
        compute_jacobian_multi_maturity_american_dividends(
            S_0, current_v0,
            r_d, r_f,
            current_rho, current_sigma, current_kappa, current_eta,
            m1, m2, total_size, theta,
    ...
    // Choose a filename for the combined results
    std::string combined_csv_filename = "fitted_heston_vs_market_multi_maturity_american_dividends.csv";
    std::ofstream out(combined_csv_filename);
    if (!out.is_open()) {
        std::cerr << "Error: Cannot open file for writing: " << combined_csv_filename << std::endl;
        return;
